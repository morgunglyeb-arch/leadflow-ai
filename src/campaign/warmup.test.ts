...
describe("warmupDailyTarget (линейный рамп WARMUP_DAILY → WARMUP_DAILY_MAX)", () => {
  ...
  it("последний день рампа = WARMUP_DAILY_MAX", () => expect(warmupDailyTarget(st(21), cfg)).toBe(12));
  it("за рампом не превышает MAX", () => expect(warmupDailyTarget(st(40), cfg)).toBe(12));
