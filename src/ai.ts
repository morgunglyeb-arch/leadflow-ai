...
POSITIONING — sell MODERN + TIME + MONEY, never a commodity they can DIY:
- NEVER lead with an automation a small business could set up itself in an afternoon with a cheap off-the-shelf app — basic auto-replies, simple appointment/booking reminders, missed-call text-back, a plain website chatbot, "we'll reply instantly" bots. These are commodities: pitching them invites "there's an app for that, I'll do it myself" and makes us look cheap and replaceable. If the only obvious gap is one of these, go a LEVEL DEEPER — a genuinely CUSTOM assistant that does the business-specific work those cheap tools cannot: understands their exact services and prices, qualifies each enquiry, handles the awkward edge cases, chases documents, and joins up the tools they already use.
- Anchor EVERY offer on the three things owners actually buy, NOT the mechanism: (1) looking MODERN — running like a bigger, cutting-edge competitor without lifting a finger; (2) saving TIME — hours of manual back-and-forth, chasing and retyping gone; (3) saving MONEY — recovered revenue or fewer paid hours. They buy being modern and getting time and money back WITHOUT having to understand any of the tech — it is done FOR them. The offer copy should make them feel current and relieved, never sold a feature.
...
- GAP-AWARE PITCHING — propose a COMPLEMENT, not a duplicate. If they already run a website chatbot OR a social-DM auto-responder (Instagram/Messenger), that text channel is handled → pitch a DEEPER piece it cannot do (still clearing the commodity bar above): a custom assistant that qualifies complex enquiries and books them, chases the paperwork a new client owes, reactivates past customers with a tailored offer, or joins up the tools they already use. Name what they have, then offer the deeper piece alongside it. Never re-sell a channel they already automate, and never down-sell them a basic reminder or auto-reply.
...
- subject: <= 50 chars and <= 5 words, LOWERCASE (lowercase out-performs Title Case in cold email — it reads as a real person's quick note, not marketing). Write it ABOUT THE BUSINESS, never about our offer: anchor it in the SAME concrete detail as the icebreaker — their service, their actual work, their town, or a page on their site (e.g. "your emergency callouts", "the quote form on your site", "leeds boiler work", "saw your reviews"). The open must be earned by RELEVANCE (it's clearly, specifically about them), NOT by a benefit hook. ❌ NEVER a benefit/pitch headline or a vague benefit-question — "missed jobs?", "instant replies?", "recovering lost jobs?", "24/7 call handling", "capturing every job?" all read as an ADVERTISEMENT and tank opens. ❌ No emojis, ALL CAPS, spam words ("free", "guarantee", "limited"), no fake "Re:"/"Fwd:". HONEST, not bait: the email's first line must deliver on whatever the subject references. Across a batch subjects must VARY and each be specific to THAT business — a subject that could fit any business is weak. Including the business name or its town is good and lifts opens.
